...
public class FileCheckTask {
	...
	//@Scheduled(cron ="0 * * * * *") //(매년) 매월 매주 매일 매시간 매분 0초마다
	...
	public void checkFiles() throws Exception{
		...
		fileList.stream().filter(vo->vo.isFileType()==true).map(vo->Paths.get("D:\\upload",vo.getUploadPath(),"s_"+vo.getUuid()+vo.getFileName())).forEach(p->fileListPaths.add(p));
